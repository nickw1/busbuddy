...
CREATE TABLE journeys(id serial PRIMARY KEY, block_ref VARCHAR(255), siri_block_ref VARCHAR(255), analysed_block_ref VARCHAR(255), origin VARCHAR(255), destination VARCHAR(255), operator_name VARCHAR(255), route VARCHAR(255), deptime TIME WITHOUT TIME ZONE, journey_code VARCHAR(255), run_days VARCHAR(255), termdays INT, holidays INT, direction VARCHAR(255), vehicle_today VARCHAR(255), rev_id INT, FOREIGN KEY (termdays) REFERENCES education_authorities(id), FOREIGN KEY (holidays) REFERENCES education_authorities(id), FOREIGN KEY (rev_id) REFERENCES tt_revisions(id));
