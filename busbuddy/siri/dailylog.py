...
try:
    with psycopg.connect(f"user={os.environ.get('DB_USER')} dbname={os.environ.get('DB_NAME')}") as conn:
        with conn.cursor() as cur:
            ...
            print(date_id)
            cur.execute("INSERT INTO daily_journey_log SELECT  j.id AS journeyid, j.siri_block_ref, j.vehicle_today, d.id as dateid FROM journeys j, dates d WHERE d.id=%s AND (j.vehicle_today IS NOT NULL OR j.siri_block_ref IS NOT NULL) ORDER BY j.vehicle_today, j.deptime", [date_id])
